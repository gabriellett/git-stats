...
const DATE_FORMAT = "MMM D, YYYY"
    ...
    , DEFAULT_ALIAS_STORE = Abs("~/.git-stats-aliases")
    , DEFAULT_DATA = {
        commits: {}, users: {}
      }
    , DEFAULT_ALIASES_DATA = {}
...
function GitStats(dataPath) {
    ...
    this.pathAliases = Abs(Deffy(dataPath, DEFAULT_ALIAS_STORE));
    this.config = {};
}
...
GitStats.prototype.record = function (data, callback) {
    ...
    function modify (err, stats) {
        ...
        var commits = stats.commits
          ...
          , today_user = users[day] = Object(users[day])
          ;

        today[data.hash] = 1;
        today_user[data.user] = today_user[data.user] ? today_user[data.user] + 1 : 1;
...
GitStats.prototype.removeCommit = function (data, callback) {
    ...
    function modify (err, stats) {
        ...
        if (!data.date) {
            ...
            IterateObject(stats.users, function (todayObj) {
                delete todayObj[data.user];
            });
        } else {
            var commits = stats.commits
              , users = stats.users
 ...
 * @return {GitStats} The `GitStats` instance.
 */
GitStats.prototype.getAliases = function (callback) {
  var self = this;

  self.get(function(err, data){
    ReadJson(self.pathAliases, function (err, aliases) {

      if (err && err.code === "ENOENT") {
        return self.saveAliases(DEFAULT_ALIASES_DATA, function (err) {
          callback(err, DEFAULT_ALIASES_DATA);
        });
      }

      if (err) { return callback(err); }
      callback(null, { stats: data, aliases: aliases });
    });
  });
...
GitStats.prototype.saveAliases = function (aliases, callback) {
    WriteJson(this.pathAliases, aliases, callback);
    return this;
};
...
GitStats.prototype.authors = function (options, callback) {
    ...
    if (options.repo) {
      ...
      function genPieData (err, data) {
        var stats = data.stats
          , users = stats.users
          , aliases = data.aliases;

        if ( options.start ){
          var start = Moment(options.start, DATE_FORMAT)
          Object.keys(users).forEach(function (date) {
            var curr = Moment(date, DATE_FORMAT)
       ...
       if ( options.end ){
          var end = Moment(options.end, DATE_FORMAT)
          Object.keys(users).forEach(function (date) {
            var curr = Moment(date, DATE_FORMAT)
        ...
        var rawCounts = [].concat.apply([], Object.keys(users).map(function (k) {
          return Object.keys(users[k]).map(function(kn) {
            var userName = aliases[kn] ? aliases[kn] : kn;
            return { user: userName, count: users[k][kn] };
          });
        })).reduce(function(sum, curr) {
          sum[curr.user] = sum[curr.user] ? sum[curr.user] + curr.count : curr.count;
          return sum;
        }, {});

        var pieData = Object.keys(rawCounts).map(function (k) {
          return { label: k, value: rawCounts[k] }
        }).sort(function (a, b){
